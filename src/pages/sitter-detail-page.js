...
export function SitterDetailsPage({componentId}) {
  ...
  const [serviceType, setServiceType] = useState(ServiceTypes.KidsPortation);
  ...
  return (
    <>
      ...
      <View style={styles.BookNowBackground}>
        <View>
          <Text style={styles.Money}>{price ? `$${price}` : 'Contact Admin'}</Text>
  ...
  async function BookingSuccessPage() {
    await Firestore().collection('bookings').add({
      ...
      invoice_number: uuid.DNS.substr(0, 14),
